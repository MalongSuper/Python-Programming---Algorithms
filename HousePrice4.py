...
print("Case 4: Only Area, Bedrooms, Bathrooms, Stories")
...
print(df[['area', 'area_scaled', 'bedrooms', 'bedrooms_scaled',
          'bathrooms_scaled', 'bathrooms', 'stories', 'stories_scaled']])
...
X_train = train_df[['area_scaled', 'bedrooms_scaled',
                    'bathrooms_scaled', 'stories_scaled']]  # Using multiple scaled features
...
# bedrooms_scaled, bathrooms_scaled, stories_scaled in this case
A = model.intercept_
B1, B2, B3, B4 = model.coef_[0], model.coef_[1], model.coef_[2], model.coef_[3]
print(f"Linear Equation: Y = {A} + ({B1} * X1) + ({B2} * X2) + ({B3} * X3) + ({B4} * X4)")

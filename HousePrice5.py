...
print("Case 5: Only Area, Bedrooms, Bathrooms, Stories, Parking")
...
print(df[['area', 'area_scaled', 'bedrooms', 'bedrooms_scaled',
          ...
          'stories_scaled', 'parking', 'parking_scaled']])
...
X_train = train_df[['area_scaled', 'bedrooms_scaled',
                    ...
                    'parking_scaled']]  # Using multiple scaled features
...
A = model.intercept_
B1, B2, B3, B4, B5 = model.coef_[0], model.coef_[1], model.coef_[2], model.coef_[3], model.coef_[4]
print(f"Linear Equation: Y = {A} + ({B1} * X1) + ({B2} * X2) + ({B3} * X3) + ({B4} * X4) + ({B5} * X5)")
